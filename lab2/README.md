...
В ходе лабораторной работы была установлена и настроена среда для работы с языком PHP, после чего была создана и запущена первая PHP-программа через встроенный веб-сервер. В работе были рассмотрены основные способы вывода данных с помощью `echo` и `print`, а также использование переменных разных типов, в частности целочисленных и строковых. Было изучено, что имена переменных в PHP начинаются со знака `$`, чувствительны к регистру, могут содержать символ нижнего подчёркивания, но не должны содержать пробелы или начинаться с цифры.

Также была рассмотрена разница между выводом данных через конкатенацию с оператором `.` и подстановкой переменных внутрь строки в двойных кавычках. Для переноса строк в браузере использовался HTML-тег `<br />`. В результате были освоены базовые правила синтаксиса PHP, работа с переменными, строками и выводом данных, а также запуск PHP-кода в браузере.
